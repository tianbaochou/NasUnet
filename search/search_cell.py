...
class SearchNetwork(object):
    ...
    def _init_logger(self):
        log_dir = '../logs/nasunet/search' + '/{}'.format(self.cfg['data']['dataset']) +\
                  '/search-{}'.format(time.strftime('%Y%m%d-%H%M'))
    ...
    def _init_dataset(self):
        ...
        data_kwargs = {'transform': data_transform}
        trainset = get_dataset(self.cfg['data']['dataset'], split='train', mode='train', **data_kwargs)

        num_train = len(trainset)
        indices = list(range(num_train))
        split = int(np.floor(self.cfg['searching']['train_portion'] * num_train))
        self.target_one_hot = trainset.target_one_hot  # indice whether use dice loss
    ...
    def _init_model(self):
        ...
        # Setup loss function
        self.criterion = SegmentationLosses(nclass=self.n_classes, mode='search',
                    type = 'dice_loss' if self.target_one_hot else None).to(self.device)
        ...
        model = NasUnetSearch(self.in_channels, init_channel, self.n_classes, depth,
                              ...
                              use_softmax_head=self.cfg['searching']['use_softmax_head'],
    ...
    def _check_resume(self):
        ...
        if self.cfg['searching']['resume'] is not None:
            if os.path.isfile(self.cfg['searching']['resume']):
                ...
                self.cur_count = checkpoint['searching']['cur_count']
                self.geno_type = checkpoint['searching']['geno_type']
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.cfg['searching']['epoch']):
            ...
            self.logger.info('epoch %d / %d lr %e', self.epoch,
                             self.cfg['searching']['epoch'], self.scheduler.get_lr()[0])
