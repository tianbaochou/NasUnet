...
class LinkNet(BaseNet):
    def __init__(
        ...
        **kwargs
        ...
        super(LinkNet, self).__init__(nclass, aux, norm_layer=nn.BatchNorm2d,**kwargs)
        ...
        self.linknet_head = nn.Conv2d(32 / feature_scale, nclass, 2, 2, 0)
    ...
    def forward(self, x):
        ...
        output = self.linknet_head(f2)

        return output

def get_linknet(dataset='pascal_voc', **kwargs):
    # infer number of classes
    from util.datasets import datasets, acronyms
    model = LinkNet(datasets[dataset.lower()].NUM_CLASS, datasets[dataset.lower()].IN_CHANNELS,
                 **kwargs)
    return model
